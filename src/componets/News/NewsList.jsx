...
function NewsList() {
  ...
  const handlePrev = (source) =>
    setPage(prev => ({ ...prev, [source]: Math.max(prev[source] - 1, 1) }));
    ...
    return (
      ...
      <Row className="mb-5">
        ...
        <Col md="12">
          ...
          <h3 className="mb-4">{source} 뉴스</h3>

        </Col>

        {paginatedNews.map((news, idx) => (

          <Col md="6" key={`${news.source}-${news.id}-${idx}`}>
      ...
      <Container>
        ...
        <Row className="mb-5">
          <Col>
            ...
            <p className="text-muted mb-0">네이버 카카오 주요 뉴스</p>
          </Col>
        </Row>
